...
Primeiramente, veremos qual distribuição o pacote gamlss recomenda para melhor ajuste aos dados

```{r,echo=TRUE}
library(gamlss)
data(cpd,package='faraway')
attach(cpd)
fitDist(actual, type="realplus") 
```
...
Como o intercepto não foi significativo, reajustaremos sem ele
